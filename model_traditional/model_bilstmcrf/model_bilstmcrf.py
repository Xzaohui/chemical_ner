...
class biLstm_model(nn.Module):
    def __init__(self):
        ...
        self.embedding = nn.Embedding(len(data_manage.word2idx),256)
        self.embedding.weight.data.copy_(torch.from_numpy(data_manage.w2v.wv.vectors))
        self.embedding.weight.requires_grad=True

        self.lstm = nn.LSTM(256,64,2,bidirectional=True)
        self.linear = nn.Linear(128,len(data_manage.BIO_lab))
